    ...
    match command {
        ...
        Command::Effort(command) => match command {
            EffortCommand::Create {
                ...
                output(json_output, &effort, || {
                    format!("Created effort {} ({})", effort.title, effort.id)
                })
            }
            EffortCommand::List => {
                let efforts = service.list_efforts().await?;
                output(json_output, &efforts, || {
                    efforts
                        .iter()
                        .map(|effort| {
                            format!(
                                "{}\t{}\t{}\tv{}",
                                effort.id, effort.slug, effort.status, effort.version
                            )
                        })
                        .collect::<Vec<_>>()
                        .join("\n")
            ...
            EffortCommand::Show { effort } => {
                ...
                output(json_output, &effort, || {
                    format!(
                        "{} ({})\nDestination: {}\nStatus: {}\nVersion: {}",
                        effort.title, effort.id, effort.destination, effort.status, effort.version
                    )
                })
            }
        },
        Command::Node(command) => match command {
            ...
            NodeCommand::List { effort, kind } => {
                ...
                let nodes: Vec<_> = graph
                    ...
                    .collect();
                output(json_output, &nodes, || {
                    nodes
                        .iter()
                        .map(|node| {
                            format!(
                                "{}\t{}\t{}\t{}",
                                node.id, node.kind, node.lifecycle, node.title
                            )
                        })
                        .collect::<Vec<_>>()
                        .join("\n")
        ...
        Command::Edge(command) => match command {
            EdgeCommand::Add {
                ...
                output(
                    json_output,
                    &json!({"edge":edge,"effort_version":version}),
                    || {
                        format!(
                            "Added {} edge {} -> {} (v{version})",
            ...
            EdgeCommand::List { effort } => {
                let (_, graph) = service.snapshot(&effort).await?;
                output(json_output, &graph.edges, || {
                    graph
                        .edges
                        .iter()
                        .map(|edge| {
                            format!(
                                "{}\t{} {} {}",
                                edge.id, edge.source_node_id, edge.edge_type, edge.target_node_id
                            )
                        })
                        .collect::<Vec<_>>()
                        .join("\n")
        ...
        Command::Frontier { effort } => {
            let status = service.status(&effort).await?;
            output(json_output, &status.frontier, || {
                status
                    .frontier
                    .iter()
                    .enumerate()
        ...
        Command::Claim(command) => match command {
            ClaimCommand::Next(args) => {
                ...
                output(json_output, &claim, || {
                    format!(
                        "Claimed {} until {} ({})",
                        claim.node_id, claim.lease_expires_at, claim.id
                    )
                })
            }
            ...
            ClaimCommand::Release {
                ...
                service
                    .release_claim(&effort, &claim, &actor, &reason)
                    .await?;
            ...
            ClaimCommand::Heartbeat {
                ...
                output(json_output, &claim, || {
                    format!(
                        "Extended claim {} until {}",
                        claim.id, claim.lease_expires_at
                    )
                })
            }
            ClaimCommand::List { effort } => {
                let (_, graph) = service.snapshot(&effort).await?;
                output(json_output, &graph.claims, || {
                    graph
                        .claims
                        .iter()
                        .map(|claim| {
                            format!(
                                "{}\t{}\t{}\t{}",
                                claim.id, claim.node_id, claim.actor_id, claim.lease_expires_at
                            )
                        })
                        .collect::<Vec<_>>()
                        .join("\n")
        ...
        Command::Resolve(args) => {
            ...
            output(
                json_output,
                &json!({"node":node,"effort_version":version}),
                || format!("Resolved {} ({}) at v{version}", node.title, node.id),
            )
        }
        Command::Reopen {
            effort,
            ...
            let (node, version) = service
                .reopen_node(&effort, &node, &actor, &reason, expected_version)
                .await?;
            output(
                json_output,
                &json!({"node":node,"effort_version":version}),
                || format!("Reopened {} ({}) at v{version}", node.title, node.id),
            )
        }
        Command::Invalidate(command) => match command {
            InvalidateCommand::Preview {
                effort,
                node,
                target,
            } => {
            ...
            InvalidateCommand::Commit {
                ...
                let (preview, version) = service
                    .commit_invalidation(&effort, &node, target, &actor, &reason, expected_version)
                    .await?;
                output(
                    json_output,
                    &json!({"preview":preview,"effort_version":version}),
        ...
        Command::Fog(command) => match command {
            FogCommand::Add {
                ...
                output(
                    json_output,
                    &json!({"fog":fog,"effort_version":version}),
                    || format!("Added fog patch {} ({}) at v{version}", fog.title, fog.id),
                )
            }
            FogCommand::Graduate {
                effort,
                ...
                output(
                    json_output,
                    &json!({"fog":fog,"graduated_to":to,"effort_version":version}),
                    || format!("Graduated fog patch {fog} at v{version}"),
                )
            }
            FogCommand::List { effort } => {
                let (_, graph) = service.snapshot(&effort).await?;
                output(json_output, &graph.fog_patches, || {
                    graph
                        .fog_patches
                        .iter()
                        .map(|fog| format!("{}\t{}\t{}", fog.id, fog.status, fog.title))
                        .collect::<Vec<_>>()
                        .join("\n")
                })
            }
        ...
        Command::Source(command) => match command {
            SourceCommand::Add {
                ...
                output(
                    json_output,
                    &json!({"source":source,"effort_version":version}),
                    || {
                        format!(
                            "Added source {} ({}) at v{version}",
                            source.title, source.id
                        )
                    },
                )
            }
            SourceCommand::Attach {
                effort,
                node,
                source,
                relationship,
                ...
                output(
                    json_output,
                    &json!({"node":node,"source":source,"relationship":relationship,"effort_version":version}),
                    || format!("Attached source {source} to {node} at v{version}"),
                )
            }
            SourceCommand::List { effort } => {
                let sources = service.sources(&effort).await?;
                output(json_output, &sources, || {
                    sources
                        .iter()
                        .map(|source| {
                            format!(
                                "{}\t{}\t{}\t{}",
                                source.id, source.kind, source.trust, source.title
                            )
                        })
                        .collect::<Vec<_>>()
                        .join("\n")
        ...
        Command::Criterion {
            ...
            output(
                json_output,
                &json!({"criterion":criterion,"effort_version":version}),
                || format!("Added criterion {} at v{version}", criterion.criterion_type),
            )
        }
        Command::Contradiction {
            ...
            output(
                json_output,
                &json!({"finding":finding,"effort_version":version}),
                || format!("Proposed contradiction {} at v{version}", finding.id),
            )
        }
        Command::History { effort, node } => {
            if let Some(node) = node {
                let revisions = service.node_history(&effort, &node).await?;
                output(json_output, &revisions, || {
                    revisions
                        .iter()
                        .map(|revision| {
                            format!(
                                ...
                                revision.revision,
                                revision.created_at,
                                revision.reason.as_deref().unwrap_or("")
                            )
                        })
                        .collect::<Vec<_>>()
                        .join("\n")
        ...
        Command::Lint { effort } => {
            let status = service.status(&effort).await?;
            output(json_output, &status.lint, || {
                if status.lint.is_empty() {
                    "No lint findings.".into()
                } else {
                    status
                        .lint
                        .iter()
        ...
        Command::Readiness { effort } => {
            let status = service.status(&effort).await?;
            output(json_output, &status.readiness, || {
                render_readiness(&status.readiness)
            })
        }
        Command::Export {
            effort,
    ...
    let (node, version) = service
        .add_node(AddNode {
            ...
            node: NewNode {
                ...
                cost_of_wrong: args.cost_of_wrong,
            },
            actor_id: args.actor,
...
async fn package(service: &Service, effort: &str, include_events: bool) -> Result<PortableEffort> {
    let (effort_value, graph, sources, events) = if include_events {
...
fn render_status(status: &threadmark_application::EffortStatusView) -> String {
    format!(
        "{} — {}\nReadiness: {}\nVersion: {}\nFrontier: {}\nActive fog: {}\nActive findings: {}\nLint findings: {}",
        status.effort.title,
        status.effort.destination,
        if status.readiness.ready {
            "READY"
        } else {
            "NOT READY"
        },
        status.effort.version,
        status.frontier.len(),
        status.active_fog,
...
fn render_readiness(readiness: &threadmark_domain::ReadinessReport) -> String {
    ...
    lines.extend(readiness.results.iter().map(|result| {
        format!(
            "{} {} — {}",
...
fn render_invalidation(preview: &threadmark_domain::InvalidationPreview) -> String {
    ...
    let mut lines = preview
        ...
        .map(|change| {
            format!(
                "{}: {} -> {} ({})",
                change.node_id, change.from, change.to, change.reason
            )
        })
        .collect::<Vec<_>>();
