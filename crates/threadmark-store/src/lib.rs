...
use sqlx::{Row, Sqlite, SqlitePool, Transaction, sqlite::{SqliteConnectOptions, SqlitePoolOptions, SqliteRow}};
use thiserror::Error;
...
impl Store {
    ...
    pub async fn create_effort(&self, effort: &Effort, event: &AuditEvent) -> Result<(), StoreError> {
    ...
    pub async fn get_effort(&self, workspace_id: &str, selector: &str) -> Result<Effort, StoreError> {
        let row = sqlx::query("SELECT * FROM efforts WHERE workspace_id = ? AND (id = ? OR slug = ?)")
            .bind(workspace_id)
            .bind(selector)
            .bind(selector)
            .fetch_optional(&self.pool)
    ...
    pub async fn update_node(
        ...
        sqlx::query("UPDATE nodes SET title=?,summary=?,lifecycle=?,validity=?,confidence=?,confidence_reason=?,reversibility=?,impact=?,uncertainty=?,cost_of_wrong=?,current_revision=?,updated_at=? WHERE id=?")
            ...
            .execute(&mut *tx).await?;
        if let Some(revision) = revision { insert_revision(&mut tx, revision).await?; }
    ...
    pub async fn insert_claim(&self, claim: &Claim, now: &str, event: &AuditEvent) -> Result<(), StoreError> {
        let mut tx = self.pool.begin().await?;
        ...
        sqlx::query("UPDATE nodes SET lifecycle='in_progress',updated_at=? WHERE id=? AND lifecycle='open'")
            .bind(now).bind(&claim.node_id).execute(&mut *tx).await?;
        insert_event(&mut tx, event).await?;
        tx.commit().await?;
        Ok(())
    }

    pub async fn release_claim(&self, claim_id: &str, now: &str, reason: &str, event: &AuditEvent) -> Result<(), StoreError> {
        let mut tx = self.pool.begin().await?;
        let row = sqlx::query("SELECT node_id FROM claims WHERE id=? AND released_at IS NULL")
            .bind(claim_id).fetch_optional(&mut *tx).await?.ok_or(StoreError::NotFound)?;
        let node_id: String = row.get("node_id");
        sqlx::query("UPDATE claims SET released_at=?,release_reason=? WHERE id=?")
            .bind(now).bind(reason).bind(claim_id).execute(&mut *tx).await?;
        sqlx::query("UPDATE nodes SET lifecycle='open',updated_at=? WHERE id=? AND lifecycle='in_progress'")
            .bind(now).bind(&node_id).execute(&mut *tx).await?;
        insert_event(&mut tx, event).await?;
        tx.commit().await?;
        Ok(())
    }

    pub async fn heartbeat_claim(&self, claim_id: &str, session_id: &str, heartbeat: &str, expires: &str) -> Result<Claim, StoreError> {
        ...
        if result.rows_affected() != 1 { return Err(StoreError::NotFound); }
        let row = sqlx::query("SELECT * FROM claims WHERE id=?").bind(claim_id).fetch_one(&self.pool).await?;
        Ok(row_to_claim(row))
    }

    pub async fn insert_fog(&self, fog: &FogPatch, event: &AuditEvent, expected_version: i64) -> Result<i64, StoreError> {
        let mut tx = self.pool.begin().await?;
    ...
    pub async fn apply_invalidation(
        ...
        for change in &preview.changes {
            sqlx::query("UPDATE nodes SET validity=?,updated_at=? WHERE id=? AND effort_id=?")
                .bind(change.to.as_str()).bind(now).bind(&change.node_id).bind(effort_id)
                .execute(&mut *tx).await?;
        }
    ...
    pub async fn insert_source(&self, source: &Source, event: &AuditEvent, expected_version: i64) -> Result<i64, StoreError> {
        let mut tx = self.pool.begin().await?;
    ...
    pub async fn attach_source(&self, effort_id: &str, node_id: &str, source_id: &str, relationship: &str, event: &AuditEvent, expected_version: i64, now: &str) -> Result<i64, StoreError> {
        let mut tx = self.pool.begin().await?;
        check_version(&mut tx, effort_id, expected_version).await?;
        sqlx::query("INSERT INTO node_sources(node_id,source_id,relationship) VALUES(?,?,?)")
            .bind(node_id).bind(source_id).bind(relationship).execute(&mut *tx).await?;
        insert_event(&mut tx, event).await?;
        let version = bump_version(&mut tx, effort_id, now).await?;
        tx.commit().await?;
        Ok(version)
    ...
    pub async fn insert_criterion(&self, criterion: &ExitCriterion, event: &AuditEvent, expected_version: i64) -> Result<i64, StoreError> {
        let mut tx = self.pool.begin().await?;
    ...
    pub async fn insert_finding(&self, finding: &Finding, event: &AuditEvent, expected_version: i64) -> Result<i64, StoreError> {
        let mut tx = self.pool.begin().await?;
    ...
    pub async fn graduate_fog(&self, effort_id: &str, fog_id: &str, node_ids: &[String], event: &AuditEvent, expected_version: i64, now: &str) -> Result<i64, StoreError> {
        let mut tx = self.pool.begin().await?;
        check_version(&mut tx, effort_id, expected_version).await?;
        ...
        for node_id in node_ids {
            sqlx::query("INSERT INTO fog_graduations(fog_id,node_id) VALUES(?,?)")
                .bind(fog_id).bind(node_id).execute(&mut *tx).await?;
        }
        insert_event(&mut tx, event).await?;
        let version = bump_version(&mut tx, effort_id, now).await?;
        tx.commit().await?;
    ...
    pub async fn snapshot(&self, effort_id: &str) -> Result<GraphSnapshot, StoreError> {
        ...
        let edges = sqlx::query("SELECT * FROM edges WHERE effort_id=? ORDER BY created_at")
            .bind(effort_id).fetch_all(&self.pool).await?.into_iter().map(row_to_edge).collect::<Result<_,_>>()?;
        ...
        let findings = sqlx::query("SELECT * FROM findings WHERE effort_id=? ORDER BY created_at")
            .bind(effort_id).fetch_all(&self.pool).await?.into_iter().map(row_to_finding).collect::<Result<_,_>>()?;
        let exit_criteria = sqlx::query("SELECT * FROM exit_criteria WHERE effort_id=? ORDER BY created_at")
            .bind(effort_id).fetch_all(&self.pool).await?.into_iter().map(row_to_criterion).collect::<Result<_,_>>()?;
        ...
        for row in source_rows { node_source_ids.entry(row.get("node_id")).or_insert_with(Vec::new).push(row.get("source_id")); }
        Ok(GraphSnapshot { nodes, edges, claims, fog_patches, findings, exit_criteria, node_source_ids })
    }
    ...
    pub async fn list_fog(&self, effort_id: &str) -> Result<Vec<FogPatch>, StoreError> {
        ...
        for row in rows {
            ...
            let graduated_to = sqlx::query("SELECT node_id FROM fog_graduations WHERE fog_id=? ORDER BY node_id")
                .bind(&id).fetch_all(&self.pool).await?.into_iter().map(|row| row.get("node_id")).collect();
            patches.push(FogPatch {
                id,
                effort_id: row.get("effort_id"), title: row.get("title"), description: row.get("description"),
                anchor_node_id: row.get("anchor_node_id"), status: parse("fog.status", row.get::<String,_>("status"))?,
                graduated_to, created_at: row.get("created_at"), updated_at: row.get("updated_at"),
            });
        }
        Ok(patches)
    }
...
async fn check_version(tx: &mut Transaction<'_, Sqlite>, effort_id: &str, expected: i64) -> Result<(), StoreError> {
    let row = sqlx::query("SELECT version FROM efforts WHERE id=?")
        .bind(effort_id).fetch_optional(&mut **tx).await?.ok_or(StoreError::NotFound)?;
    let actual: i64 = row.get("version");
    if actual == expected { Ok(()) } else { Err(StoreError::VersionConflict { expected, actual }) }
}

async fn bump_version(tx: &mut Transaction<'_, Sqlite>, effort_id: &str, now: &str) -> Result<i64, StoreError> {
    sqlx::query("UPDATE efforts SET version=version+1,updated_at=? WHERE id=?")
        .bind(now).bind(effort_id).execute(&mut **tx).await?;
    let row = sqlx::query("SELECT version FROM efforts WHERE id=?").bind(effort_id).fetch_one(&mut **tx).await?;
    Ok(row.get("version"))
}

async fn insert_revision(tx: &mut Transaction<'_, Sqlite>, revision: &NodeRevision) -> Result<(), StoreError> {
...
async fn insert_event(tx: &mut Transaction<'_, Sqlite>, event: &AuditEvent) -> Result<(), StoreError> {
...
fn row_to_effort(row: SqliteRow) -> Result<Effort, StoreError> {
    Ok(Effort { id: row.get("id"), workspace_id: row.get("workspace_id"), slug: row.get("slug"), title: row.get("title"),
        destination: row.get("destination"), scope_notes: row.get("scope_notes"), status: parse("effort.status", row.get::<String,_>("status"))?,
...
fn row_to_node(row: SqliteRow) -> Result<Node, StoreError> {
    Ok(Node { id: row.get("id"), effort_id: row.get("effort_id"), kind: parse("node.kind", row.get::<String,_>("kind"))?,
        title: row.get("title"), summary: row.get("summary"), lifecycle: parse("node.lifecycle", row.get::<String,_>("lifecycle"))?,
        validity: parse("node.validity", row.get::<String,_>("validity"))?, confidence: parse_optional("node.confidence", row.get("confidence"))?,
        confidence_reason: row.get("confidence_reason"), reversibility: parse_optional("node.reversibility", row.get("reversibility"))?,
        impact: parse_optional("node.impact", row.get("impact"))?, uncertainty: parse_optional("node.uncertainty", row.get("uncertainty"))?,
        cost_of_wrong: parse_optional("node.cost_of_wrong", row.get("cost_of_wrong"))?, current_revision: row.get("current_revision"),
        body: row.get("body"), payload: parse_json("node.payload", row.get("payload_json"))?, created_at: row.get("created_at"), updated_at: row.get("updated_at") })
}

fn row_to_revision(row: SqliteRow) -> Result<NodeRevision, StoreError> {
    Ok(NodeRevision { node_id: row.get("node_id"), revision: row.get("revision"), body: row.get("body"),
        payload: parse_json("revision.payload", row.get("payload_json"))?, reason: row.get("reason"), actor_id: row.get("actor_id"),
        session_id: row.get("session_id"), created_at: row.get("created_at") })
}

fn row_to_edge(row: SqliteRow) -> Result<Edge, StoreError> {
    Ok(Edge { id: row.get("id"), effort_id: row.get("effort_id"), source_node_id: row.get("source_node_id"),
        edge_type: parse("edge.type", row.get::<String,_>("type"))?, target_node_id: row.get("target_node_id"), rationale: row.get("rationale"),
        created_by: row.get("created_by"), created_at: row.get("created_at") })
}

fn row_to_claim(row: SqliteRow) -> Claim {
    Claim { id: row.get("id"), node_id: row.get("node_id"), actor_id: row.get("actor_id"), session_id: row.get("session_id"),
        claimed_at: row.get("claimed_at"), heartbeat_at: row.get("heartbeat_at"), lease_expires_at: row.get("lease_expires_at"),
        released_at: row.get("released_at"), release_reason: row.get("release_reason") }
}

fn row_to_finding(row: SqliteRow) -> Result<Finding, StoreError> {
    Ok(Finding { id: row.get("id"), effort_id: row.get("effort_id"), finding_type: parse("finding.type", row.get::<String,_>("type"))?,
        severity: parse("finding.severity", row.get::<String,_>("severity"))?, status: parse("finding.status", row.get::<String,_>("status"))?,
        title: row.get("title"), detail: row.get("detail"), related_nodes: parse_json("finding.related_nodes", row.get("related_nodes_json"))?,
        proposed_by: row.get("proposed_by"), adjudication: row.get("adjudication"), created_at: row.get("created_at"), updated_at: row.get("updated_at") })
}

fn row_to_criterion(row: SqliteRow) -> Result<ExitCriterion, StoreError> {
    Ok(ExitCriterion { id: row.get("id"), effort_id: row.get("effort_id"), criterion_type: row.get("type"),
        config: parse_json("criterion.config", row.get("config_json"))?, required: row.get::<i64,_>("required") != 0, created_at: row.get("created_at") })
}

fn row_to_event(row: SqliteRow) -> Result<AuditEvent, StoreError> {
    Ok(AuditEvent { id: row.get("id"), effort_id: row.get("effort_id"), actor_id: row.get("actor_id"), session_id: row.get("session_id"),
        event_type: row.get("event_type"), entity_type: row.get("entity_type"), entity_id: row.get("entity_id"),
        before: parse_optional_json("event.before", row.get("before_json"))?, after: parse_optional_json("event.after", row.get("after_json"))?,
        reason: row.get("reason"), occurred_at: row.get("occurred_at") })
}

fn row_to_source(row: SqliteRow) -> Result<Source, StoreError> {
    Ok(Source { id: row.get("id"), effort_id: row.get("effort_id"), kind: parse("source.kind", row.get::<String,_>("kind"))?,
        uri: row.get("uri"), title: row.get("title"), retrieved_at: row.get("retrieved_at"), observed_at: row.get("observed_at"),
        content_hash: row.get("content_hash"), excerpt: row.get("excerpt"), metadata: parse_json("source.metadata", row.get("metadata_json"))?,
        trust: parse("source.trust", row.get::<String,_>("trust"))?, created_at: row.get("created_at") })
}

fn parse<T: FromStr<Err = String>>(field: &'static str, value: String) -> Result<T, StoreError> {
    value.parse().map_err(|_| StoreError::InvalidEnum { field, value })
}

fn parse_optional<T: FromStr<Err = String>>(field: &'static str, value: Option<String>) -> Result<Option<T>, StoreError> {
    value.map(|value| parse(field, value)).transpose()
}

fn parse_json<T: serde::de::DeserializeOwned>(field: &'static str, value: String) -> Result<T, StoreError> {
    serde_json::from_str(&value).map_err(|source| StoreError::InvalidJson { field, source })
}

fn parse_optional_json(field: &'static str, value: Option<String>) -> Result<Option<Value>, StoreError> {
